...
#     tmod = Trans_mod.Train_test(dataset='samson', device=device, skip_train=False, save=True, index=ind, data_print=False)
#     tmod.run(smry=False)

print("\nSelected device:", device, end="\n\n")
tmod = Trans_mod.Train_test(dataset='jasper', device=device, skip_train=False, save=True, data_print=True)
tmod.run(smry=False)
